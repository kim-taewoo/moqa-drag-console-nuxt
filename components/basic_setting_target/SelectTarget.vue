  ...
  <v-layout justify-center wrap>
    <v-flex xs12 class="text-xs-right">
      ...
      <v-dialog max-width="290" persistent v-model="addTargetDialog">
        <v-btn slot="activator" class="primary">현재 설정을 맞춤타겟에 추가</v-btn>
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn class="green--text" flat @click.native="addTargetDialog = false">취소</v-btn>
            <v-btn class="green--text" flat @click.native="addTargetDialog = false">저장</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
      <v-btn class="subheading primary" @click.native="setTarget">
        적용하기
      </v-btn>
    </v-flex>
...
export default {
  ...
  methods: {
    setTarget() {
      let form = {
        targetAge: this.selectedAge,
        targetLocation: this.selectedArea,
        targetMarryYn: this.selectedMarry,
        targetFamilly: this.selectedFamily,
        targetJob: this.selectedJob,
        targetEducation: this.selectedEducation,
        targetGender: this.selectedGender,
        targetMobileComp: this.selectedCellphone,
        targetPrivacyIncome: this.selectedIndIncome,
        targetFamillyIncome: this.selectedFamIncome
      }
      this.$emit('targetSet', form)
    }
  },
  data() {
    return {
